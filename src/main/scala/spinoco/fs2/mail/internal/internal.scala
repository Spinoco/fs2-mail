...
package object internal {
  ...
  /**
    * Behaves like [[Stream.takeThrough]], but resulting stream will not end until `predicate` yields to false,
    * draining all elements where `predicate` still yields to true.
    *
  ...
  def takeThroughDrain[F[_] : Concurrent, A](predicate: A => Boolean): Pipe[F, A, A] = { source =>
    Stream.eval(cats.effect.std.Queue.synchronous[F, Option[Either[Throwable, A]]]).flatMap{ feedQueue =>

      // dequeue and propagate errors to downstream
      def dequeue:Stream[F, A] = {
        Stream.fromQueueNoneTerminated(feedQueue).flatMap {
      ...
      Stream.eval(Ref.of[F, Boolean](false)).flatMap{ finishedRef =>
        Stream.eval(Concurrent[F].start(
          ...
          .evalMap( a => feedQueue.offer(Some(Right(a))))
          .compile.drain.attempt.flatMap { r =>
            finishedRef.set(true) >> feedQueue.offer(r.left.toOption.map(Left(_)))
          }
