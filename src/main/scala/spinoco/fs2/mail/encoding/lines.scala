...
object lines {
  ...
  def by[F[_]](separator: ByteVector): Pipe[F, Byte, Byte] = {
      ...
      _.receiveOption {
        case Some((ch, t)) =>
          ...
          if (idx < 0) go(data)(t)
            ...
            if (t0.isEmpty) Pull.output(ByteVectorChunk(h)) >> go(ByteVector.empty)(t)
            else Pull.output(ByteVectorChunk(h)) >> go(t0.drop(separator.size))(t)
          }
  ...
  def byCrLf[F[_]]: Pipe[F, Byte, Byte] =
    by(ByteVector.view("\r\n".getBytes))
